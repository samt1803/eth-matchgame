...
it('should be possible to create a new game', () => contractInstance.startNewGame(accounts[1])
// assert that the transaction was successful
  .then(result => assert.equal(result.receipt.status, '0x01', 'transaction to create account was not successful')),
);
...
it('only the owner should be able to UNpause the game', () => contractInstance.unpause({ from: accounts[1] })
  ...
  .then(result => assert.equal(result.receipt.status, '0x01', 'unpausing by the owner didn\'t work'))
  // it shoould no be possible to create a game again
  .then(() => contractInstance.startNewGame(accounts[1]))
  .then(result => assert.equal(result.receipt.status, '0x01', 'creating game after unpausing didn\'t work')),
);
