...
before(() => {
  return new Promise((resolve) => {
    contract('Matchgame', (_accounts) => {
      accounts = _accounts
      Matchgame.deployed().then(instance => {
...
it('should be possible to create a new game', () => {
  contractInstance.startNewGame(accounts[1])
  // assert that the transaction was successful
  // depending on the version 0x01 or 0x1 is returned as successful, 0x0 or 0x00 as failed
    .then(result => assert.isTrue(result.receipt.status, 'transaction to create account was not successful'))
}
)
...
it('should be possible to get the game state. It should be the initial gamestate', () => contractInstance.getGame()
  .then((result) => {
    ...
    assert.equal(result[2].toNumber(), 0, 'someone is a winner after initalizing the game')
  }))
...
it('should be possible to make a move as player 2. Then the gamestate should have changed', () => contractInstance.makeMove(0, 1, { from: accounts[1] })
  // assert that the transaction was successful
  .then(result => {
    assert.isTrue(result.receipt.status === true, 'transaction to create account was not successful')})
